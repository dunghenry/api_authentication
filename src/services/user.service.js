...
class userService {
    ...
    static async verifyOtp({ email, otp }) {
        try {
            ...
            const isValidOtp = await otpService.validOtp({ otp, hashedOtp: lastOtp.otp });
            if (!isValidOtp) {
                return {
                    status: 401,
                    message: 'Invalid OTP',
            ...
            if (isValidOtp && email === lastOtp.email) {
                ...
                const hashedPassword = await bcrypt.hash(userService.password, salt);
    ...
    static async register({ email, password, username }) {
        ...
        if (user) {
            ...
            return {
                status: 201,
                message: insertedOtp === 1 ? 'Created otp successfully!' : 'Created otp failed!',
            }
        }
